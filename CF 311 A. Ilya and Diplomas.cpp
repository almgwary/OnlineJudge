    ...
    /**
    * @name	  : CF 311 A. Ilya and Diplomas
    * @author : almgwary
    * @date   : 30 jun 2015
    * @time	  : M
    */
...
int main()
{
    long long int n ,f,s,t,fmin,fmax,smin,smax,tmin,tmax;
    cin>>n>>fmin>>fmax>>smin>>smax>>tmin>>tmax;
    f=fmin; fmax-=fmin; n-=fmin;
    s=smin; smax-=smin; n-=smin;
    t=tmin; tmax-=tmin; n-=tmin;
    if(n>0){
        f+=(min(fmax,n));
        n=max((long long int)0,n-fmax);
    }
    if(n>0){
        s+=(min(smax,n));
        n=max((long long int)0,n-smax);
    }
    if(n>0){
        t+=(min(tmax,n));
        n=max((long long int)0,n-tmax);
    }
    cout<<f<<" "<<s<<" "<<t<<endl;
    return 0;
}
